...
Copyright (C) 2019, 2020, 2021, 2022, 2023 The poke authors.
...
** Debugging poke with GDB
   ...
   uses SIGPWR for internal purposes, in order to stop all threads
   before starting collecting memory.  Normally this signal is
   delivered right after the system is powered on and booted, and it
   is almost always ignored by processes.  However, when you run poke
   under GDB, GDB will stop once SIGPWR is signaled to the current
   thread.  If this happens, just (c)ontinue.
