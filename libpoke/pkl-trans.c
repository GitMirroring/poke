...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_pr_loop_stmt)
{
  ...
  if (iterator)
    {
      ...
      //      printf ("XXX PUSH FRAME pkl_transl_pr_loop_stmt ITERATOR\n");
  ...
  if (head)
    {
      //      printf ("XXX PUSH FRAME pkl_transl_pr_loop_stmt HEAD\n");
      PKL_TRANS_ENV = pkl_env_push_frame (PKL_TRANS_ENV);
      if (PKL_TRANS_FUNCTION)
        PKL_TRANS_FUNCTION->back++;
  ...
  if (iterator || head)
    {
      //      printf ("XXX POP FRAME pkl_transl_pr_loop_stmt\n");
...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_pr_comp_stmt)
{
  ...
  if (!PKL_AST_COMP_STMT_FRAMELESS_P (comp_stmt))
    {
      //      printf ("XXX PUSH FRAME pkl_transl_pr_comp_stmt\n");
      PKL_TRANS_ENV = pkl_env_push_frame (PKL_TRANS_ENV);
      if (PKL_TRANS_FUNCTION)
        {
          if (PKL_PASS_PARENT
...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_ps_comp_stmt)
{
  ...
  if (!PKL_AST_COMP_STMT_FRAMELESS_P (comp_stmt))
    {
      //      printf ("XXX POP FRAME pkl_transl_ps_comp_stmt\n");
      PKL_TRANS_ENV = pkl_env_pop_frame (PKL_TRANS_ENV);
      if (PKL_TRANS_FUNCTION)
        {
          PKL_TRANS_FUNCTION->back--;
...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_pr_try_stmt)
{
  ...
  if (arg)
    {
      //      printf ("XXX PUSH FRAME pkl_transl_pr_try_stmt\n");
      PKL_TRANS_ENV = pkl_env_push_frame (PKL_TRANS_ENV);
  ...
  if (arg)
    {
      //      printf ("XXX POP FRAME pkl_transl_pr_try_stmt\n");
      PKL_TRANS_ENV = pkl_env_pop_frame (PKL_TRANS_ENV);
    }
...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_pr_func)
{
  ...
  /* printf ("XXX PUSH FRAME pkl_transl_pr_func '%s'\n", */
  /*         PKL_AST_FUNC_NAME (func) ? */
  /*         PKL_AST_FUNC_NAME (func) : "lambda"); */
  PKL_TRANS_ENV = pkl_env_push_frame (PKL_TRANS_ENV);
  ...
  for (arg = args; arg; arg = PKL_AST_CHAIN (arg))
    PKL_PASS_SUBPASS (arg);
  PKL_PASS_SUBPASS (body);
  //  printf ("XXX POP FRAME pkl_transl_pr_func\n");
  PKL_TRANS_ENV = pkl_env_pop_frame (PKL_TRANS_ENV);
...
{
  //  printf ("XXX PUSH FRAME pkl_transl_pr_type_struct\n");
  PKL_TRANS_ENV = pkl_env_push_frame (PKL_TRANS_ENV);
  if (PKL_TRANS_FUNCTION)
    PKL_TRANS_FUNCTION->back++;
  ...
  /*                     "ent_size", &back, &over)) */
  /*   printf ("XXX TYPE STRUCT '%s' BACK=%d OVER=%d\n", */
  /*           PKL_AST_IDENTIFIER_POINTER (PKL_AST_TYPE_NAME (PKL_PASS_NODE)), back, over); */
...
{
  //  printf ("XXX POP FRAME pkl_transl_pr_type_struct\n");
  PKL_TRANS_ENV = pkl_env_pop_frame (PKL_TRANS_ENV);
  if (PKL_TRANS_FUNCTION)
    PKL_TRANS_FUNCTION->back--;
}
...
PKL_PHASE_BEGIN_HANDLER (pkl_transl_ps_var)
{
  ...
  if (!pkl_env_lookup (PKL_TRANS_ENV, PKL_ENV_NS_MAIN,
    ...
    /* printf ("XXX %s VAR '%s' BACK=%d OVER=%d\n", */
    /*         PKL_AST_LOC_VALID (PKL_AST_LOC (var)) ? pkl_ast_format_loc ("", PKL_AST_LOC (var)) : "", */
    /*         PKL_AST_IDENTIFIER_POINTER (PKL_AST_VAR_NAME (PKL_PASS_NODE)), */
    /*         PKL_AST_VAR_BACK (var), */
    /*         PKL_AST_VAR_OVER (var)); */
