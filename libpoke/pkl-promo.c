...
PKL_PHASE_BEGIN_HANDLER (pkl_promo_ps_op_binary)
{
  ...
  switch (PKL_AST_EXP_CODE (exp))
    ...
    case PKL_AST_OP_AND:
    case PKL_AST_OP_OR:
    case PKL_AST_OP_IMPL:
      {
        pkl_ast_node op1_type;
        pkl_ast_node op2_type;

        op1_type = PKL_AST_TYPE (PKL_AST_EXP_OPERAND (exp, 0));
        op2_type = PKL_AST_TYPE (PKL_AST_EXP_OPERAND (exp, 1));
        ...
        /* Promote operands to a type that is wide enough for both of them.  */
        type = pkl_type_integral_promote (PKL_PASS_AST, op1_type, op2_type);
      }
      break;
    default:
      type = PKL_AST_TYPE (exp);
    }
  ...
  if (PKL_AST_TYPE_CODE (type) == PKL_TYPE_INTEGRAL)
    {
      ...
      if (PKL_AST_EXP_CODE (node) == PKL_AST_OP_NOT)
        {
          pkl_ast_node exp_type = PKL_AST_TYPE (PKL_AST_EXP_OPERAND (node, 0));

          if (PKL_AST_TYPE_CODE (exp_type) == PKL_TYPE_STRUCT)
            type = PKL_AST_TYPE_S_ITYPE (exp_type);
        }

      size = PKL_AST_TYPE_I_SIZE (type);
      signed_p = PKL_AST_TYPE_I_SIGNED_P (type);
...
struct pkl_phase pkl_phase_promo =
  {
    ...
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FEQ, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FNE, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FLT, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FGT, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FLE, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FGE, pkl_promo_ps_op_frela),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FNEG, pkl_promo_ps_op_unary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FADD, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FSUB, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FMUL, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FPOW, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FDIV, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FCEILDIV, pkl_promo_ps_op_binary),
    PKL_PHASE_PS_OP_HANDLER (PKL_AST_OP_FMOD, pkl_promo_ps_op_binary),
