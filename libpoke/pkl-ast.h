...
#define PKL_AST_FUNC_PROGRAM_GC_HANDLE(AST) ((AST)->func.program_gc_handle)
...
/* PKL_AST_TYPE nodes represent type expressions.
   ...
   CODE contains the kind of the type, as defined in the
   pkl_ast_type_code enumeration above, perhaps after a chain of alias
   types.  CODE2 is like CODE but it refers to the code of the
   immediate type, without traversing, which may be a type alias.
   ...
   NAMED_P contains a flag that tells whether this type node is
   aliased, i.e. it has appeared as an initial in a declaration.  This
   means this node can only be referred via an alias.

   In alias types, NAME is the name of the aliased type encoded in an
   IDENTIFIER node.  TYPE is the aliased type.  NEXT_IN_DECL is the
   next alias node that refers to the same type declaration or NULL.
...
#define PKL_AST_TYPE_NAMED_P(AST) (pkl_ast_type_resolv (AST)->type.named_p)
#define PKL_AST_TYPE_L_NAME(AST) ((AST)->type.val.alias.name)
#define PKL_AST_TYPE_L_TYPE(AST) ((AST)->type.val.alias.type)
#define PKL_AST_TYPE_L_NEXT_IN_DECL(AST) ((AST)->type.val.alias.next_in_decl)
...
struct pkl_ast_type
{
  ...
  union
  {
    struct
    {
      union pkl_ast_node *name;
      union pkl_ast_node *type;
      union pkl_ast_node *next_in_decl;
    } alias;
...
/* PKL_AST_DECL nodes represent the declaration of a named entity:
   ...
   TYPE_NAMES is a list of alias PKL_AST_TYPE nodes, whose names shall
   be updated should this declaration be renamed.
...
#define PKL_AST_DECL_TYPE_NAMES(AST) ((AST)->decl.type_names)
...
void pkl_ast_add_type_name_to_decl (pkl_ast_node decl, pkl_ast_node type_name);
void pkl_ast_rename_decl (pkl_ast ast, pkl_ast_node decl, const char *new_name);
